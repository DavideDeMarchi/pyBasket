...
class ThrowMap(widgets.HBox):
    ...
    def createControls(self):
        ...
        self.added_pixels_width  = 0
        self.added_pixels_height = 0

        # Output widget to display all the throws
        self.outdraw = widgets.Output(layout=Layout(width= 'calc(%fvw + %dpx)' % (self._width,self.added_pixels_width), 
        ...
        self.img = v.Img(width='%fvw'%self._width, height='%fvh'%self._height, contain=True, src=colors.image2Base64(self.imgBackground), style_='background-color: black;')
        with self.outdraw:
            display(self.img)
    ...
    def select(self,
        ...
        self.outselect = widgets.Output(layout=Layout(width= 'calc(%fvw + %dpx)' % (w,self.added_pixels_width), 
                                      height='calc(%fvh + %dpx)' % (h,self.added_pixels_height),
                                      margin='0px 0px 0px 0px')) #, border='1px solid red'))
        ...
        self.imgSelect = v.Img(width='%fvw'%w, height='%fvh'%h, contain=True, src=colors.image2Base64(self.imgSelectBackground), style_='background-color: black;')
        with self.outselect:
        ...
        self.dlg = dialogGeneric.dialogGeneric(title='Select throw position', text='', titleheight=30,
    ...
    def displayThrows(self, tdf, background=True):
        ...
        if background :
            if self.current_df is not None:
                ...
                T1ok  = self.current_df[(self.current_df['team']=='Team')&(self.current_df['event_name']=='T1ok') ].shape[0]
                T1err = self.current_df[(self.current_df['team']=='Team')&(self.current_df['event_name']=='T1err')].shape[0]
                T2ok  = self.current_df[(self.current_df['team']=='Team')&(self.current_df['event_name']=='T2ok') ].shape[0]
                T2err = self.current_df[(self.current_df['team']=='Team')&(self.current_df['event_name']=='T2err')].shape[0]
                T3ok  = self.current_df[(self.current_df['team']=='Team')&(self.current_df['event_name']=='T3ok') ].shape[0]
                T3err = self.current_df[(self.current_df['team']=='Team')&(self.current_df['event_name']=='T3err')].shape[0]
                ...
                fontBold   = ImageFont.truetype('fonts/Roboto-Bold.ttf',    fontsize+2)
                ...
                if self.current_player is not None:
                    ...
                    draw.text((x3,y+4*dy), '%d'%Stats.points(self.current_df, name), textcolor, font=fontNormal)
                    
                    draw.text((x2,y+5*dy), 'VAL:', textcolor, font=fontNormal)
                    draw.text((x3,y+5*dy), '%d'%Stats.value(self.current_df, name), textcolor, font=fontNormal)

                    draw.text((x2,y+6*dy), 'OER:', textcolor, font=fontNormal)
                    draw.text((x3,y+6*dy), '%.2f'%Stats.oer(self.current_df, name), textcolor, font=fontNormal)

                    draw.text((x2,y+7*dy), 'VIR:', textcolor, font=fontNormal)
                    draw.text((x3,y+7*dy), '%.2f'%Stats.vir(self.current_df, name, self.game.players_info), textcolor, font=fontNormal)
                    
                    draw.text((x2,y+8*dy), '+/-:', textcolor, font=fontNormal)
                    draw.text((x3,y+8*dy), '%d'%Stats.plusminus(name, self.game.players_info), textcolor, font=fontNormal)
                    
                    draw.text((x2,y+9*dy), 'TS:', textcolor, font=fontNormal)
                    draw.text((x3,y+9*dy), '%d%%'%Stats.trueshooting(self.current_df, name), textcolor, font=fontNormal)
    ...
    def updateThrows(self, df, player_name=None, background=True):
        if df is not None:
            ...
            if player_name is None:                   # No players selected --> Throws for the Team
                ...
                tdf = df[(df['team']=='Team')&(df['player']==player_name)]
                if background:
    ...
    def handle_event_select(self, event):
        ...
        if self.onclick is not None:
            ...
            x = 100.0 * px / widthpixel
            if x > 100: x = 100

            y = 100.0 * py / heightpixel
            if y > 100: y = 100
